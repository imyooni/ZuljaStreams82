...
const streamers = [
    ...
    ['annayapiano', 'Piano', ['English'],['T']],
    ['yena', 'Piano', ['Korean','English'],['T']],
    ...
    ['vivaceconfuoco', 'Piano', ['Korean','English'],['T']],
    ['han_padahati', 'Piano', ['Chinese','English'],['T']],
    ['jer_roque', 'Piano', ['English'],['T']],
    ['harlos_0517', 'Piano', ['Chinese','English'],['T']],
...
async function getStreamerInfo(streamerName, platform) {
  if (platform[0] === 'C') {
    ...
    try {
      const response = await fetch(`https://zuljababot.onrender.com/chzzk/channels/${platform[1]}`);
      if (!response.ok) {
    ...
    try {
      const response = await fetch(`https://zuljababot.onrender.com/api/twitch/${streamerName}`);
      if (!response.ok) {
